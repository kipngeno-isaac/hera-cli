...
- **Hera config:** `HERA_SANDBOX=auto` → resolves to `bwrap` ("fs confined to cwd, no network")
...
- **network is disabled** by default (`HERA_SANDBOX_NET=1` re-enables it);
...
the full reason→act loop (`write_file` → `run_bash`) with the sandbox active. The script's own
output, as run by the agent:
...
|---|---|
| default (`net off`) | exit `6` (blocked) |
| `HERA_SANDBOX_NET=1` | `http=200` (reachable) |

Network is off by default and cleanly re-enabled when explicitly allowed.
...
off unless opted in, layered on top of the approval gate and allowlist/denylist.
